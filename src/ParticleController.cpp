...
void ParticleController::addParticle(int xi, int yi, int res, Vec2f rotationVector)
{
	...
	Particle* p = new Particle(Vec2f(x, y), rotationVector);
...
void ParticleController::addParticles(int amt)
{
	for(int i=0; i<amt; i++){
		for(int i=0; i<amt; i++){
			...
			mParticles.push_back(new Particle(Vec2f(x, y), Vec2f(1.0f, 0.0f)));
		}
	}
}
...
void ParticleController::printFlowLookUpTable(){
	if(mFlowLookUpTable != NULL){
		for (int x = 0; x < mXRes; ++x){ 
			for(int y = 0; y < mYRes; ++y){
